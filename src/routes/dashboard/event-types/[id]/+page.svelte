<script lang="ts">
	...
	let category = $state(data.eventType.category || 'Consultation');
	let description = $state(data.eventType.description || '');
	let status = $state<'live' | 'paused'>(data.eventType.is_active === 1 ? 'live' : 'paused');
	let pricingType = $state<'complimentary' | 'paid'>(data.eventType.is_free_only === 1 ? 'complimentary' : 'paid');
	let priceInr = $state<number>(data.eventType.price_inr || 999);
	let selectedDurations = $state<number[]>(data.eventType.durations || [30]);
	let selectedExpertIds = $state<string[]>(data.assignedExpertIds || []);
...
<svelte:head>
	<title>Edit {data.eventType.name} | Neubofy™</title>
</svelte:head>
...
<div class="p-6 sm:p-10 max-w-4xl mx-auto space-y-8 animate-fade-in">
	...
	<div class="flex items-center justify-between">
		...
		<span class="text-xs text-zinc-500 font-mono">Editing Service</span>
	</div>

	<!-- Header -->
	<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
		<div>
			<h1 class="text-2xl sm:text-3xl font-bold text-white tracking-tight">Edit Consultation Service</h1>
			<p class="text-xs sm:text-sm text-zinc-400 mt-1">
				Update service details, manage session duration tiers, toggle Live/Paused status, and assign specialists.
			</p>
		</div>
		...
		<div>
			{#if status === 'live'}
				<span class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 shadow-[0_0_12px_rgba(16,185,129,0.2)]">
					<span class="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
					Status: Live (Active)
				</span>
			{:else}
				<span class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-amber-500/15 text-amber-400 border border-amber-500/30">
		...
		>
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
				...
				<div>
					...
					<p class="text-[11px] text-zinc-500 mt-1">Direct client link: <code>booking.neubofy.in/{slug}</code></p>
				</div>
			</div>
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
				<div>
					...
					<input
						...
						name="category"
						bind:value={category}
						placeholder="e.g. Advisory, Architecture, Review"
						class="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/15 text-sm text-white placeholder-zinc-500 focus:outline-none focus:border-blue-500 transition-all"
					/>
				</div>
			...
			<div class="pt-4 border-t border-white/10">
				<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
					<div>
						<label class="block text-xs font-bold uppercase tracking-wider text-zinc-200">
							Assigned Specialists / Experts *
						</label>
						<p class="text-[11px] text-zinc-400 mt-0.5">
							Clients will only see and book specialists who are checked below.
						</p>
					</div>
				...
				{:else}
					<div class="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs">
						⚠️ No team members found in the organization.
					</div>
				{/if}
			</div>
			...
			<div class="pt-6 border-t border-white/10 flex items-center justify-end gap-4">
				...
				>
					<span>{isSaving ? 'Saving Changes...' : 'Save Consultation Service →'}</span>
				</button>
			</div>
		</form>
	</div>
