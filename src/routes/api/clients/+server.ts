/**
 * Organization & Expert Clients API Endpoint
 * Returns clients and their consultation history, accessible only to authenticated experts and admins.
 */
...
export const GET = async (event: RequestEvent) => {
	...
	const url = event.url;
	const workspaceMode = url.searchParams.get('workspace') || (isAdmin ? 'org' : 'personal');

	try {
		// Filter by organization, and if personal mode, only bookings for this expert
		let queryStr = `
		...
		// Aggregate bookings by unique client (using email as primary identifier, enriched with Firebase UID)
		...
		for (const b of bookings) {
			...
			// Update phone or UID if later booking has them
		...
		console.error('[api:clients] Failed to fetch clients directory:', err);
