...
<div class="w-72 border-r border-white/10 flex flex-col flex-shrink-0 bg-black/20">
	...
	<div class="flex-1 p-6">
		<div class="mb-6">
			{#if user?.profileImage}
				<img src={user.profileImage} alt={user.name} class="w-12 h-12 rounded-full object-cover mb-3 border border-white/20" />
			{:else}
				<div class="w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-lg mb-3 border border-white/20" style="background-color: {brandColor}">
					{user?.name?.charAt(0) || 'M'}
				</div>
			{/if}
			<p class="text-xs font-semibold uppercase tracking-wider text-blue-400 mb-1">{user?.name || 'Neubofy Consultant'}</p>
			<h1 class="text-xl font-bold text-white leading-snug">{eventType?.name || 'Consultation'}</h1>
		</div>
		...
		{#if eventType?.description}
			<div class="mt-6 pt-6 border-t border-white/10">
				<div class="text-xs text-zinc-400 leading-relaxed max-w-none [&_p]:my-2 [&_ul]:my-2 [&_ol]:my-2 [&_li]:my-1">
		...
		{#if selectedDate && selectedSlot}
			<div class="mt-6 pt-6 border-t border-white/10">
				<div class="flex items-center gap-3 text-xs">
					...
					<div>
						<p class="font-bold text-white">{formatTime(selectedSlot.start)} - {formatTime(selectedSlot.end)}</p>
						<p class="text-zinc-400 mt-0.5">{formatSelectedDate(selectedDate)}</p>
					</div>
				</div>
			</div>
		{/if}
